...
@UIApplicationMain
class PQGAppDelegate : UIResponder, UIApplicationDelegate {

  var window: UIWindow?
  
  let store: PQGPersistStore = PQGPersistStore(file: PQGPersistStore.URLForDocument("geonoter.db"))

  func applicationDidFinishLaunching(_ application: UIApplication) {
    
    UserDefaults.standard().register([
      "LocationsUseGeocoder": true,
    ...
    if let error = LocationHelper.sharedHelper().requestIfNotYetDone() {
      NSLog("Error! %@", error)
      let alert = UIAlertController(title: error.localizedDescription, message: "You will not be able to add points at this time", preferredStyle: .alert)
      window?.rootViewController?.present(alert, animated: true, completion: nil)
    } else {
      CLLocationManager().requestWhenInUseAuthorization()
    }
    
    Foursquare2.setupFoursquare(withClientId: "QDD4H5DPDAQZBCMDPY2ZCKIW01WLVSO44NKVSDIYD3ULA10R", secret: "5AE4ZA1RGY0PQ15ADS51UFRF4MNYGJSRMGUUW2XZZ1O3LEVZ", callbackURL: "x-com-pftqg-geonoter")
  }
