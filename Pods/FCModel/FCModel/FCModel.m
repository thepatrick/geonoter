...
+ (id)primaryKeyValueForNewInstance
{
    ...
    if (databaseIsOpen && g_tablesUsingAutoIncrementEmulation && [g_tablesUsingAutoIncrementEmulation containsObject:[self tableForClass]]) {
...
- (FCModelSaveResult)save
{
    ...
    NSString *tableName = [self.class tableForClass];
...
+ (NSString *)expandQuery:(NSString *)query
{
    ...
    NSLog(@"$PK %@ / $T %@", g_primaryKeyFieldName[self], [self.class tableForClass]);
    query = [query stringByReplacingOccurrencesOfString:@"$PK" withString:g_primaryKeyFieldName[self]];
    return [query stringByReplacingOccurrencesOfString:@"$T" withString:[self.class tableForClass]];
}
...
static NSMutableDictionary *FCModelTableClassMap;

+ (NSMutableDictionary*)tableClassMap {
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    FCModelTableClassMap = [NSMutableDictionary dictionary];
  });
  return FCModelTableClassMap;
}

+ (void)setClassName:(NSString*)className forTableName:(NSString*)tableName
{
  [[self tableClassMap] setValue:className forKey:tableName];
}

+ (Class)classForTable:(NSString*)tableName
{
  NSString *className = [self tableClassMap][tableName];
  if(!className) {
    className = tableName;
  }
  NSLog(@"classForTable %@ => %@", tableName, className);
  return NSClassFromString(className);
}

+ (NSString*)tableForClass
{
  NSString *className = NSStringFromClass(self);
  NSString *tableName = [[[FCModel tableClassMap] keysOfEntriesPassingTest:^BOOL(id key, id obj, BOOL *stop) {
    ...
    [g_databaseQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([tablesRS next]) {
          ...
          NSLog(@"found table %@", tableName);
            Class tableModelClass = [self classForTable:tableName];
          
          if (! tableModelClass || ! [tableModelClass isSubclassOfClass:self]) {
            NSLog(@"tableModelClass %@", tableModelClass);
            continue;
          }
            ...
            NSLog(@"query %@", [NSString stringWithFormat: @"PRAGMA table_info('%@')", tableName]);
            ...
            while ([columnsRS next]) {
              ...
              NSLog(@"isPK %@, %@", fieldName, isPK == 1 ? @"YES" : @"NO");
